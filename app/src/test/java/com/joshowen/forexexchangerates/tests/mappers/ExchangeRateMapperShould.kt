...
class ExchangeRateMapperShould : BaseUnitTest() {
    ...
    fun maintainsExpectedUSDValuePreMapping() {
        assertEquals(
            expectedResponse.first { it.currency == CurrencyType.US_DOLLARS }.price,
            mappedResponse.first { it.currency == CurrencyType.US_DOLLARS }.price
        )
    }
    ...
    fun maintainsExpectedEurosValuePreMapping() {
        assertEquals(
            expectedResponse.first { it.currency == CurrencyType.EUROS }.price,
            mappedResponse.first { it.currency == CurrencyType.EUROS }.price
        )
    }
    ...
    fun maintainsExpectedJapaneseYenValuePreMapping() {
        assertEquals(
            expectedResponse.first { it.currency == CurrencyType.JAPANESE_YEN }.price,
            mappedResponse.first { it.currency == CurrencyType.JAPANESE_YEN }.price
        )
    }
    ...
    fun maintainsExpectedGreatBritishPoundsValuePreMapping() {
        assertEquals(
            expectedResponse.first { it.currency == CurrencyType.GREAT_BRITISH_POUNDS }.price,
            mappedResponse.first { it.currency == CurrencyType.GREAT_BRITISH_POUNDS }.price
        )
    }
    ...
    fun maintainsExpectedAustralianDollarsValuePreMapping() {
        assertEquals(
            expectedResponse.first { it.currency == CurrencyType.AUSTRALIAN_DOLLARS }.price,
            expectedResponse.first { it.currency == CurrencyType.AUSTRALIAN_DOLLARS }.price
        )
    }
    ...
    fun maintainsExpectedCanadianDollarsValuePreMapping() {
        assertEquals(
            expectedResponse.first { it.currency == CurrencyType.CANADIAN_DOLLARS }.price,
            expectedResponse.first { it.currency == CurrencyType.CANADIAN_DOLLARS }.price
        )
    }
    ...
    fun maintainsExpectedSwissFrancValuePreMapping() {
        assertEquals(
            expectedResponse.first { it.currency == CurrencyType.SWISS_FRANC }.price,
            expectedResponse.first { it.currency == CurrencyType.SWISS_FRANC }.price
        )
    }
    ...
    fun maintainsExpectedChineseYuanValuePreMapping() {
        assertEquals(
            expectedResponse.first { it.currency == CurrencyType.CHINESE_YUAN }.price,
            expectedResponse.first { it.currency == CurrencyType.CHINESE_YUAN }.price
        )
    }
    ...
    fun maintainsExpectedSwedishKronaValuePreMapping() {
        assertEquals(
            expectedResponse.first { it.currency == CurrencyType.SWEDISH_KRONA }.price,
            expectedResponse.first { it.currency == CurrencyType.SWEDISH_KRONA }.price
        )
    }
